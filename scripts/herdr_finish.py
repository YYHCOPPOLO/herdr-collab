...
DEFAULT_MAX_BOUNCES = 3  # failure bounces before the lead is added to the notify
...
def write_run_log(job: str, run_log: list, proc=None, note: str = "", bounces: int = 0) -> Path:
...
def bounce_count(log: Path) -> int:
    try:
        return int(json.loads(log.read_text(encoding="utf-8")).get("bounces") or 0)
    except Exception:
        return 0


def bounce_targets(on_fail: list, on_pass: list, bounces: int, max_bounces: int) -> tuple[list, bool]:
    """Past max_bounces the lead side joins the failure notify. Never blind-retry."""
    targets = list(on_fail)
    escalated = bounces > max_bounces
    if escalated:
        for target in on_pass:
            if target not in targets:
                targets.append(target)
    return targets, escalated


def bounce(job: str, clerk: str, run_log: list, on_fail: list, on_pass: list,
           max_bounces: int, message: str, proc=None, note: str = "") -> None:
    """Record one bounce and notify. Only execution/GitHub failures come here;
    validation rejects and aborts do not count."""
    bounces = bounce_count(jobs_dir() / f"{job}.run.json") + 1
    log = write_run_log(job, run_log, proc=proc, note=note, bounces=bounces)
    targets, escalated = bounce_targets(on_fail, on_pass, bounces, max_bounces)
...
def validate_handshake(data: dict, job: str) -> list[str]:
    ...
    max_b = data.get("max_bounces", DEFAULT_MAX_BOUNCES)
    if isinstance(max_b, bool) or not isinstance(max_b, int) or max_b <= 0:
        problems.append(f"max_bounces 非法：{data.get('max_bounces')!r}")
...
def main() -> int:
    ...
    for command in data.get("finish_run") or []:
        ...
        try:
            ...
            bounce(job, clerk, run_log, on_fail, on_pass, max_bounces,
                   f"找不到命令：{argv[0]}。", note=f"command not found: {argv[0]}")
            return 127
            ...
            run_log.append(entry_log)
            bounce(job, clerk, run_log, on_fail, on_pass, max_bounces,
                   f"命令超时（{effective_timeout:.0f}s）：{command}。"
                   "需要更长时限就在回执里调大 timeout_sec。",
                   note=f"timeout after {effective_timeout}s")
            return 124
        ...
        if proc.returncode != 0:
            ...
            bounce(job, clerk, run_log, on_fail, on_pass, max_bounces,
                   ...
                   f"命令：{command} exit={proc.returncode}。修好后更新回执再 handoff。",
                   proc=proc)
    ...
    if comment and issue and repo:
        try:
            ...
            bounce(job, clerk, run_log, on_fail, on_pass, max_bounces,
                   ...
                   "网络恢复后更新回执再 handoff 即可（重跑幂等，不会重复 commit）。",
                   note=f"github comment/close failed: {exc}")
            return 1
